...
export interface GrafanaUrlParams {
   ...
   * @security This token is appended as a URL query parameter (`auth_token`), making it
   * visible in browser history, server access logs, and HTTP Referer headers. Prefer
   * session-cookie auth (the default auth-proxy flow) over this option wherever possible.
   */
  authToken?: string
